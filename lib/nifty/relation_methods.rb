...
module Nifty
  module RelationMethods #:nodoc:
    ...
    def nifty_batches(options = {})
      ...
      unless block_given?
        return to_enum(:nifty_batches, options) do
      ...
      records = start ? relation.where(table[primary_key].gteq(start)).nifty : relation.nifty
      ...
      while records.any?
        ...
        records = relation.where(table[primary_key].gt(primary_key_offset)).nifty
      end
    end
  end
end